...
This release is not production webcam eye tracking. Browser gaze estimation is present only as an experimental, feature-flagged spike behind `VITE_ENABLE_WEBGAZER`, explicit consent, and guarded `window.webgazer` access. WebGazer is not bundled, and the project does not claim medical-grade accuracy, biometric identity, real heatmaps, screenshot replay, DOM AOI detection, CAF delay, auth, deployment, export, or sharing.
...
It is not bundled, not release-ready as real gaze tracking, and not part of the default demo path.
...
AOIs are normalized rectangles where `x` and `y` are top-left coordinates and `width`/`height` are dimensions from 0 to 1. Current AOIs are demo placeholders. Screenshot uploads, DOM-derived AOIs, production webcam gaze estimation, and CAF delay are future work.
...
See [docs/v0.1-demo-release-checklist.md](docs/v0.1-demo-release-checklist.md) for the release checklist.
...
See [docs/mvp-roadmap.md](docs/mvp-roadmap.md) for completed baseline status and next milestones. The near-term path is to harden the synthetic demo release, then improve validation, demo reset/seed instructions, copy around the experimental browser gaze mode, and deterministic screenshots/GIFs.
