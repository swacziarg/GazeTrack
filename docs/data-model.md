...
This document separates the current SQLite-backed `v0.1-demo` model from the future production schema direction. The live backend uses a compact SQLite schema; the PostgreSQL-style tables below are a target shape, not a claim that auth, teams, production reports, heatmaps, or multi-tenant permissions are implemented.

## Current `v0.1-demo` SQLite model
...
Production note: `attention_entropy`, `gaze_dispersion`, and `heatmap_json` are future fields. They are not returned by the current `v0.1-demo` backend report.
